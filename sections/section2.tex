...
首当其冲的是把当前的下一条指令地址\textoverset{Push}{压}到栈顶，接下来是把栈顶指针加 1，然后再 \texttt{jmp} 到函数的入口\footnote{\texttt{jmp}：本质上读者可能已经发现了，\texttt{jmp} 实际上就是把某个值写入程序寄存器，这样一来 CPU 就会跳转到这个地址执行了。}。在函数结束时，我们需要先恢复栈顶指针，再把栈顶我们事先存的下一条地址\textoverset{Pop}{弹}出来，最后再 \texttt{jmp} 到这个地址。
...
% \begin{itemize}
%     \item 把数字换为立即数，\texttt{123} $\to$ \texttt{\#123}
%     \item 把 \texttt{x123} 换为内存地址，\texttt{x123} $\to$ \texttt{123}
%     \item \texttt{a += b} $\to$ \texttt{add a, b}
%     \item \texttt{a -= b} $\to$ \texttt{sub a, b}
%     \item \texttt{a *= b} $\to$ \texttt{mul a, b}
%     \item \texttt{a /= b} $\to$ \texttt{div a, b}
%     \item \texttt{a = b} $\to$ \texttt{mov a, b}
%     \item \texttt{if a != b jmp addr} $\to$ \texttt{cmp a, b} 和 \texttt{jne addr}
%     \item \texttt{if a == b jmp addr} $\to$ \texttt{cmp a, b} 和 \texttt{je addr}
%     \item \texttt{if a < b jmp addr} $\to$ \texttt{cmp a, b} 和 \texttt{jl addr}
% \end{itemize}
