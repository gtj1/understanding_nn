...
不过我想可以对解决的方法做一个简单的分类。按照参数的数量，从参数复杂到参数简单可以画出一条轴。按照模型获取经验的方式，从模型完全编码了先验经验，到通过一些例子得到经验，再到持续在与环境的互动中获取经验，可以画出另一条轴。在这里我也试图并不严谨地画出了这样一个表格：@table:mlmethods。
...
我把模型抽象成五个要素：#textOverSet("Input", "输入")、#textOverSet("Output", "输出")、#textOverSet("Architecture", "模型结构")、#textOverSet("Loss Function", "损失函数")和#textOverSet("Optimizer", "优化算法")。输入、模型架构和具体参数决定了输出如何计算，按照损失函数计算得到的损失指导模型调整具体参数，优化算法则决定了参数如何调整。当然这样的划分只是我自己的理解，而非理解神经网络的唯一方式。这里我不打算在概念之间玩文字游戏，把机器学习中的概念倒来倒去，变成一篇又臭又长，令人看完莫名其妙、不知所云、又对实践毫无益处的文章。我认为画一个图串起来（如@fig:nn_elements） 是最直观的方式。

#nn_elements<fig:nn_elements>

#h2 从输入到输出再到损失的过程通常称为#textOverSet("Forward Propagation", "正向传播")，而从损失到参数的更新过程则称为#textOverSet("Backward Propagation", "反向传播")。而这中间的模型结构常常由矩阵运算与一些#textOverSet("Activation Function", "激活函数")构成的层组成。几乎可以说众多的神经网络中，只有这种传播的方式和网络的基本组成元素是相同的，如何从这些基本元素构建出好的模型则像是搭积木一样，各有各的搭法。
