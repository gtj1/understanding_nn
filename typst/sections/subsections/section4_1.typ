...
于是前面的两个问题就有了答案：泛化性来自于数据分布中隐藏的规律。我们之所以相信模型能够举一反三，是因为我们相信：只要数据足够规律，模型就有能力学到其中的结构，而且有能力比我们人工编码的特征做得更好#footnote[比人工编码做的更好：事实上甚至比人类自己做的更好，现代的 AI 求解验证码的速度与准确度均超过了人类，见#link("https://zhuanlan.zhihu.com/p/652027321")[《一觉睡醒，AI 破解验证码的速度比我还快了》]，而当今的验证码#link("https://www.zhihu.com/question/519996527/answer/3398147109")[实际上是用来验蠢的]。]。

顺着这个思路继续思考，我们会发现许多有趣的结论。以图像为例：在真实世界中，相邻像素的颜色往往相近，一个物体的颜色也通常只在小范围内波动。而从理论上说，一张 $224 times 224$ 的 RGB 图像有 $256^{224 times 224 times 3}$ 种可能——这是一个天文数字。然而，几乎所有这些可能的图像都是无意义的噪声，真正有意义的图像只占极小一部分#footnote[有意义的数据点只占一部分：这通常称为(低维)流形假设，假设认为多维的数据点实际上处于一个维度低的多的子空间中。]。
...
推荐算法运行的基础也是如此：#link("https://www.bilibili.com/video/BV1mUbyzbEqi")[大数据是如何猜测用户喜好的呢]？每个用户可能喜欢某个视频，也可能不喜欢，让每个用户把每个视频都看一遍并打分显然不现实。光是从储存来看，记录上亿的用户观看上亿个视频的结果就已经超出了大多数系统的承受范围。所幸用户的选择并不是独立的，如果两个用户常常看相同的视频，那么他们的喜好也很可能相似，所以：对它使用低秩假设#footnote[低秩假设：指的是可以找到一个低秩的矩阵来近似给定的矩阵，写成一竖一横两个长条形矩阵的乘积。如果读者对线性代数中的秩并不熟悉，只需要理解它表达的含义与前文"有效数据只占极小一部分"相同。]吧。
...
如何构造出结构化的数据也是现代机器学习研究的重点之一。以#link("https://arxiv.org/abs/2112.10752")[Stable Diffusion]为代表的扩散模型可以说是在生成领域的伟大尝试，既然随机生成的数据并无结构，那何不试着无中生有地构造出符合人类认知的图像？它的思路十分巧妙：先获得一些符合人类认识的图像并试图加入噪声，那么只要学习怎么去掉噪声就可以了，就这样无中生有地从噪声中"恢复"出了图像。更晚些的#link("https://arxiv.org/abs/2210.02747")[流匹配]可以看作是系统化地完善了原有的扩散框架。从统计学上说，既然随机噪声与结构化图像之间最本质的差别是分布上的差异，那么结构化生成的问题就可以等价地理解为将一个随机的均匀分布迁移到另一个结构化分布的问题，将问题从样本的层面提升到了分布的层面。
