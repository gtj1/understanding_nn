...
#figure(table(
  ...
  [$arrow$ `cmp r1, r2`], [6, 18, 18, ...], [15, 18, ?, ...], [1, 2, 3, ...], [(18$eq$18,顺序执行)],
  [$arrow$ `mov 10, r0`], [6, 18, 18, ...], [15, 18, 6, ...], [1, 2, 3, ...], [将 $r_0$ 的 6 存入 $x_(10)$]
))

#h2 这个程序的执行过程是这样的：
+ 初始化：将 $r_0$ 设为 0，$r_1$ 设为起点地址（15），$r_2$ 设为终点地址（18）。
+ 循环：
  - 将 $r_1$ 指向的内存单元的值加到 $r_0$ 中
  - $r_1$ 加 1，指向下一个内存单元
  - 比较 $r_1$ 和 $r_2$ 的值，如果不相等就继续循环
+ 最后将 $r_0$ 中的结果（6）存入内存 10 号位置。
...
#v1 #h2 在上一节中，我们看到了计算机是如何执行程序的。但是我们还没有讨论数据是如何表示的。在计算机中，所有的数据都是以二进制的形式存储的。这是因为计算机的硬件是由电路组成的，而电路只能表示两种状态：高电平和低电平。因此，计算机中的数据都是由 0 和 1 组成的。

=== 整数的表示

#v1 #h2 整数是最简单的数据类型。在计算机中，整数通常使用#textOverSet("Two's Complement", "二进制补码")表示。例如，在一个 8 位的系统中，数字的表示范围是 -128 到 127。这是因为：
...
#v1 #h2 浮点数的表示比整数要复杂得多。在计算机中，浮点数通常使用#textOverSet("IEEE 754", "IEEE 754 标准")表示。这个标准定义了两种主要的格式：
- 单精度（32 位）
- 双精度（64 位）
...
#v1 #h2 字符在计算机中也是用二进制表示的。最常用的字符编码标准是 ASCII 和 Unicode。ASCII 使用 7 位二进制表示 128 个字符，包括：
- 英文字母（大小写）
- 数字
- 标点符号
- 控制字符
