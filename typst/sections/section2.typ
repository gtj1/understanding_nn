...
这一章将视角从拟合上短暂地移开，我相信理解逻辑和数据的关系多少也会帮助我们理解神经网络。读者或许好奇过，计算是如何完成的呢？在讨论这个问题之前，先来做一个约定，我们将 0 视作#textOverSet("False", "假")，1 视作#textOverSet("True", "真")#footnote[逻辑0/1：在物理上，逻辑 0 由#textOverSet("Low", "低电平")表示，逻辑 1 由#textOverSet("High", "高电平")表示，TTL 和 CMOS 电路各有多种电压标准，感兴趣的读者可以自行学习电路的知识。]。先来看看几种最简单的逻辑运算。

1. #textOverSet("Not", "非")（数学写法：$neg$，C 语言写法：`!`，Python 写法：`not`）

   非是一元运算符，它只有一个输入，输出与输入相反，其中
   $
       neg 0 = 1, neg 1 = 0
   $
   也就是说 $neg x = 1 - x$，$x$ 与 $neg x$ 是互补的。如果你看逻辑 0, 1 仍然感觉不太自然，你可以把它想成 False = not True, True = not False。

2. #textOverSet("And", "与")（数学写法：$wedge$，C 语言写法：`&&`，Python 写法：`and`）
   ...
   $
   这与乘法的结果是一样的，所以有时也会省去和的符号，使用 $x y$ 表示 $x wedge y$。

3. #textOverSet("Or", "或")（数学写法：$vee$，C 语言写法：`||`，Python 写法：`or`）
...
$
    mat(
        delim: none,
        dots.h, phantom(0), 1, phantom(0), color(gray.lighten(50%), dots.h);
        +, color(red, 1), 0, color(blue, 1), color(gray.lighten(50%), dots.h);
        hdash.repeat(5);
        dots.h, phantom(0), color(red, 1), phantom(0), color(gray.lighten(50%), dots.h)
    )
$
...
// - 程序如何编译出来：#link("https://repository.unikom.ac.id/48769/1/Compilers%20-%20Principles,%20Techniques,%20and%20Tools%20(2006).pdf")[《编译原理》](_Compilers: Principles, Techniques, and Tools_)
// - 计算机的结构：#link("https://www.cs.sfu.ca/~ashriram/Courses/CS295/assets/books/CSAPP_2016.pdf")[《深入理解计算机系统》](_Computer Systems: A Programmer's Perspective_)
// - 程序的结构：#link("https://web.mit.edu/6.001/6.037/sicp.pdf")[《计算机程序的构造和解释》](_Structure and Interpretation of Computer Programs_)
...
先解释一下这些指令名称的含义：
// - `mov`：mov*e 的缩写，将一个数值从一个地方移动到另一个地方。
// - `add`, `sub`, `mul`, `div`：*add*, *sub*tract, *mul*tiply, *div*ide 的缩写，加减乘除。
// - `jmp`, `je`, `jne`, `jl`：*j*u*mp*, *j*ump if *e*qual, *j*ump if *n*ot *e*qual, *j*ump if *l*ess 的缩写，分别为跳转、当等于时跳转、当不等于时跳转、当小于时跳转。
// - `cmp`：*cmp*are 的缩写，比较。
...
#table(
  ...
  align: left,
  [指令], [内存状态],
  [#h(1em) (initial)], [$[x, y, ?, dots]$],
  [$arrow$ `mov 2, 0`], [$[x, y, x, dots]$],
  [$arrow$ `mov 0, 1`], [$[y, y, x, dots]$],
  [$arrow$ `mov 1, 2`], [$[y, x, x, dots]$]
)
